...
public class Functions {
    ...
    public static String addDatabaseConnectorModulePath(String endOfUri, RenderContext renderContext) {
        ...
        try {
            ...
            try {
                JCRSessionWrapper session = renderContext.getMainResource().getNode().getSession();
                QueryResultWrapper execute = session.getWorkspace().getQueryManager().createQuery("select * from [dcmix:directivesDefinition] where localname() = '" + StringUtils.substringAfterLast(endOfUri, "/") + "'", Query.JCR_SQL2).execute();
